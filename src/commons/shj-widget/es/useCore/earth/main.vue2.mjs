import { defineComponent as j, useSlots as M, computed as i, onMounted as D, onBeforeUnmount as X, openBlock as Y, createElementBlock as Z, createElementVNode as G, unref as R, renderSlot as U } from "vue";
import { nanoid as V } from "../../node_modules/.pnpm/nanoid@4.0.2/node_modules/nanoid/index.browser.mjs";
import { l as $ } from "../../node_modules/.pnpm/lodash@4.17.21/node_modules/lodash/lodash.mjs";
...
const J = { class: "earth-wrap" }, K = ["id"], re = /* @__PURE__ */ j({
  ...
  setup(S, { expose: C, emit: w }) {
    const s = V(), l = M(), t = S, v = i(() => ({
      ...
      earthParameter: L.value,
      apertureParameter: B.value,
      cloudCoverParameter: k.value,
      ...
      composerParameter: P.value
    })), E = w;
    let a;
    const T = () => new Promise((r) => {
      new q(document.getElementById(s), x.value).start().then((e) => {
        a = e, r(e);
      });
    }), N = async () => {
      ...
      r && r.classList.remove("showLabel"), T().then((e) => {
        setTimeout(async () => {
          await N(), await O(), await _(), await A(), await E("on-success"), a.accessAnimation().then((o) => {
